...
const getAllStyleParties = CatchAsyncErrors(async (req, res, next) => {
  try {
...
const createStyleParty = CatchAsyncErrors(async (req, res, next) => {
  try {
...
const updateStyleParty = CatchAsyncErrors(async (req, res, next) => {
  try {
    const stylePartyId = req.params.id;
    const { name, description, status } = req.body;
    const styleParty = await stylePartyModel.findByIdAndUpdate(
      stylePartyId,
      req.body
    );
